...
class NewsSummarizer(private val llm: LLMClient) {
        ...
        try {
            ...
            if (!isRetryMode) {
                try {
                    ...
                    val blitzTopics = globalApprovedTopics.filter { it.isBlitz }
                    val nonBlitzTopics = globalApprovedTopics.filter { !it.isBlitz }

                    Log.d(TAG, "REDUCTION_DEBUG: [Stage C.1] Entering smartMergeTopics. Non-Blitz size: ${nonBlitzTopics.size}, Blitz size: ${blitzTopics.size}")

                    val mergedTopics = if (filterTopics && nonBlitzTopics.isNotEmpty()) {
                        MewsRepository.setUpdatingState(UpdatingState.FILTERING)
                        try {
                            val result = smartMergeTopics(nonBlitzTopics, currentLanguage, 0, bannedWords)
                            Log.d(TAG, "REDUCTION_DEBUG: [Stage C.2] Exit smartMergeTopics. Merged result size: ${result.size}")
                            result + blitzTopics
                        } catch (e: Exception) {
                    ...
                    Log.d(TAG, "REDUCTION_DEBUG: [Stage D] Final topics slicing. Before window filter: ${mergedTopics.size} -> After: ${finalTopics.size} (Limit was: $extractionLimit)")
                    finalTopics.forEach { saveTopicToDb(it, extractionTime) }
                    ...
                    while (iterator.hasNext() && batch.size < 10) {
                        ...
                        val isDuplicate = processedTopicsSignatures.any { (procTitle, procKeywords) ->
                            ...
                            for (tk in candidateTags) {
                                for (pk in procTags) {
                                    if (tk.equals(pk, ignoreCase = true) || TextComparator.areSimilar(tk.lowercase(), pk.lowercase(), 0.9)) {
                                        keywordMatches++
                                        break
                                    }
                                }
                            }
                            val titleMatch = TextComparator.areSimilar(candidate.title, procTitle, 0.65)
                            val matchesFound = keywordMatches > 3 || titleMatch
                            if (matchesFound) {
                                Log.d(TAG, "REDUCTION_DEBUG: [Stage E] Drop topic [${candidate.id}] '${candidate.title}' as DUPLICATE. Cause: TitleMatch=$titleMatch, KeyMatchCount=$keywordMatches (Matched with: '$procTitle')")
                            }
                            matchesFound
                        }
                                        ...
                                        if (bestMatchId != null) {
                                            ...
                                            if (parentTopic != null && isParentUnread && isParentRecent) {
                                                ...
                                                MewsRepository.updateTitle(
                                                    id = bestMatchId,
                                                    newEventTime = parentTopic.updateTime,
    ...
    private suspend fun processNewsBuffer(
        ...
        Log.d(TAG, "REDUCTION_DEBUG: [Stage A] Raw extracted count in this buffer execution: ${allExtracted.size}")
        val prevGlobalSize = globalApprovedTopics.size
        mergeIntoGlobal(allExtracted, globalApprovedTopics)
        Log.d(TAG, "REDUCTION_DEBUG: [Stage B] mergeIntoGlobal: was $prevGlobalSize -> now ${globalApprovedTopics.size}")
    }
    ...
    private fun mergeIntoGlobal(newTopics: List<Topics>, globalCache: MutableList<Topics>) {
        ...
        newTopics.forEach { candidate ->
            val existingIndex = globalCache.indexOfFirst { existing ->
                ...
                if (existing.isBlitz == candidate.isBlitz && existing.keywords.size >= keywordMatches && candidate.keywords.size >= keywordMatches) {
                    var matchCount = 0
    ...
    private suspend fun adaptiveExtractTopics(batch: List<GroupedMessage>, maxLimit: Int, lang: String, depth: Int, banned: String): List<Topics> {
        ...
        try {
            ...
            val prompt = """
                ...
                2. Сюжетная кластеризация: Объединяй события в одну тему только при наличии прямой причинно-следственной связи. Избегайте объединения независимых происшествий на основе схожей локации или общей сферы деятельности. Крайне нежелательно связывать в одну тему некрологи/смерти публичных лиц и параллельные расследования/дела, если они не являются прямым следствием друг друга.
    ...
    private suspend fun saveTopicToDb(t: Topics, extractionTime: Long) {
            ...
            for (i in 0 until jsonArray.length()) {
                try {
                    ...
                    if (summary.trim() == "REJECTED" || summary.trim().length <= 15) {
                        if (original != null) {
                            Log.d(TAG, "REDUCTION_DEBUG: [Stage F] Topic [${original.first.id}] '${original.first.title}' deleted (REJECTED or too short summary length: ${summary.trim().length})")
    ...
    private suspend fun sumTopicsBatch(llm: LLMClient, data: List<Triple<Topics, List<Message>, String>>, banned: String, lang: String, isBlitz: Boolean): String {
        ...
        val prompt = if (isBlitz) {
            ...
            Инструкции:
            1. ДОСТОВЕРНОСТЬ И ПРИОРИТЕТ ИСТОЧНИКА: Строй обзор исключительно на фактах из поля news_content. Использование внешних знаний допускается только для восполнения пробелов в контексте или пояснения терминов. Факты из источника имеют абсолютный приоритет: запрещено противоречить тексту, изменять или заменять указанные в нем имена, должности, статусы или связи участников.
            2. КРИТИЧЕСКИЙ ЗАПРЕТ: Запрещено генерировать данные, имена, номера и последствия, отсутствующие во входящем JSON.
            3. УМНАЯ ФИЛЬТРАЦИЯ: Если новость содержит запрещенные темы или спам ('$banned') вместе с разрешенным контекстом, ПОЛНОСТЬЮ ИГНОРИРУЙ/ВЫРЕЗАЙ запрещенные детали и переписывай новость исключительно на основе оставшейся разрешенной информации. Возвращай строго слово REJECTED в поле "summary" только в крайнем случае — если после удаления запрещенного контента в новости вообще не осталось полезного смысла.
            4. ЯЗЫК: $lang.
            5. ТОЧНОСТЬ ДАННЫХ: СТРОГО запрещено обобщать статистику, суммы, даты и конкретные показатели. Переноси их из исходного текста без изменений. Наиболее важные заявления и цитаты передавай исключительно прямой речью в кавычках («»), без пересказа своими словами.
            ...
            """
            Ты — аналитик и обозреватель. Твоя цель — написать подробный лонгрид или развернутую статью, объем которой прямо пропорционален объему и насыщенности входящей фактической информации. Запрещено искусственно сжимать текст.
            Инструкции:
            1. СТИЛЬ: Smart Casual с упором на литературность и читаемость. Строгая новостная объективность без эмоциональных оценок.
            2. ДОСТОВЕРНОСТЬ И ПРИОРИТЕТ ИСТОЧНИКА: Строй обзор исключительно на фактах из поля news_content. Использование внешних знаний допускается только для восполнения пробелов в контексте или пояснения терминов. При этом факты из источника имеют абсолютный приоритет: запрещено противоречить тексту, изменять или заменять указанные в нем имена, должности, статусы или связи участников (даже если внешние источники или твоя база знаний утверждают иное).
            3. ПРОПОРЦИОНАЛЬНЫЙ ОБЪЕМ (БЕЗ УТЕРИ ДАННЫХ): Объем итогового текста должен соответствовать плотности и количеству входящей информации. Если во входящих материалах содержится большое количество важных фактов, событий и значимых деталей, пиши развернутый и подробный текст. Запрещено усекать или опускать важные подробности ради искусственной лаконичности. Сокращай текст только за счет удаления речевой избыточности и повторов, полностью сохраняя всю информационную наполненность.
            4. КРИТИЧЕСКИЙ ЗАПРЕТ: Запрещено генерировать данные, имена, номера и последствия, отсутствующие во входящем JSON.
            5. ФОРМАТИРОВАНИЕ: Выделяй жирным шрифтом ключевые имена, даты, организации и термины. Разделяй длинные или сложные сюжеты логическими абзацами.
            6. УМНАЯ ФИЛЬТРАЦИЯ: Если новость содержит запрещенные темы или спам ('$banned') вместе с разрешенным контекстом, ПОЛНОСТЬЮ ИГНОРИРУЙ/ВЫРЕЗАЙ запрещенные детали и переписывай новость исключительно на основе оставшейся разрешенной информации. Возвращай строго слово REJECTED в поле "summary" только в крайнем случае — если после удаления запрещенного контента в новости вообще не осталось полезного смысла.
            7. ЯЗЫК: $lang.
            8. ПРЕДОХРАНИТЕЛЬ СЮЖЕТОВ: Если во входящем контенте (news_content) содержатся разнородные важные события, ты обязан детально описать каждое из них в отдельных абзацах. Одно событие не должно вытеснять или поглощать другое.
            9. ТОЧНОСТЬ ДАННЫХ: СТРОГО запрещено обобщать статистику, суммы, даты и конкретные показатели. Переноси их из исходного текста без изменений. Наиболее важные заявления и цитаты передавай исключительно прямой речью в кавычках («»), без пересказа своими словами.
            ФОРМАТ ОТВЕТА (СТРОГО JSON):
            [{"id": <id из input>, "title": "<заголовок>", "summary": "<отформатированный текст статьи или строго "REJECTED">"}]
            Ввод: $jsonInput
            """.trimIndent()
...
//        val minKeywordMatches = 2
        ...
        for (historyItem in history) {
            ...
            val titlesThreshold = TextComparator.countThreshold(
                topic.title.lowercase(),
                historyItem.title.lowercase()
            )
            val summaryThreshold = if (summaryToCompare != null) TextComparator.countThreshold(
                TextSanitizer.sanitize(summaryToCompare.lowercase()),
                TextSanitizer.sanitize(historyItem.summary.lowercase())
            ) else 0.0
            currentScore += titlesThreshold + summaryThreshold * 2

            if (currentScore >= aimedScore && currentScore > maxScore) {
