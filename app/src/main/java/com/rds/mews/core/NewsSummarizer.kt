    ...
    private suspend fun adaptiveExtractTopics(batch: List<GroupedMessage>, maxLimit: Int, lang: String): List<Topics> {
        ...
        try {
            ...
            val prompt = """
                ...
                СТРОГИЙ ФИЛЬТР: Игнорируй любую рекламу, промо-коды, розыгрыши, коммерческие интеграции, сообщения с маркировкой 'erid' и призывы подписаться на каналы. Если сообщение является рекламой — не включай его в результат.
